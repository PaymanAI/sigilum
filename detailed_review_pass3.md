...
| P3-002 | P1 | security, bug, install, scripts | open | `openclaw/install-openclaw-sigilum.sh`, `scripts/sigilum-auth.sh` | Owner JWT can be persisted in OpenClaw config even when `sigilum-authz-notify` is disabled, and installer output can expose the JWT value. Reduce token exposure and keep disabled-hook runtime free of owner token by default. |
...
- [ ] Fix P3-002, update this file, test, stage, commit.
